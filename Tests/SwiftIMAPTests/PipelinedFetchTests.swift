...
struct PipelinedCommandDispatcherTests {
    ...
    func pendingCount() {
        ...
        let firstHandler = PipelinedFetchPartHandler(promise: firstPromise)
        dispatcher.register(tag: "A001", handler: firstHandler)
        #expect(dispatcher.pendingCount == 1)

        let secondPromise = eventLoop.makePromise(of: Data.self)
        let secondHandler = PipelinedFetchPartHandler(promise: secondPromise)
        dispatcher.register(tag: "A002", handler: secondHandler)
        #expect(dispatcher.pendingCount == 2)

        // Clean up — resolve promises to avoid NIO "leaking promise" fatal error
        firstHandler.fail(IMAPError.timeout)
        secondHandler.fail(IMAPError.timeout)
    }
    ...
    func failIndividualHandlers() async {
        ...
        // Fail first handler only
        firstHandler.fail(IMAPError.timeout)

        do {
            _ = try await firstPromise.futureResult.get()
            Issue.record("first handler should have failed")
        } catch {
            // Expected
        }

        // second handler should still be pending — fail it too
        secondHandler.fail(IMAPError.connectionFailed("test"))

        do {
            _ = try await secondPromise.futureResult.get()
            Issue.record("second handler should have failed")
        } catch {
            // Expected
        }
    }
