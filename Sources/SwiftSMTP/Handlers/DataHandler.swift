import Foundation
import NIOCore
import Logging

/**
 Handler for the DATA command response
 */
public final class DataHandler: BaseSMTPHandler<Bool> {
    
    /**
     Process a response from the server
     - Parameter response: The response to process
     - Returns: Whether the handler is complete
     */
    override public func processResponse(_ response: SMTPResponse) -> Bool {
        logger.debug("Received DATA response: \(response.code) \(response.message)")
        
        // 3xx responses are considered successful for DATA command (server is ready for content)
        if response.code >= 300 && response.code < 400 {
            logger.info("Server ready for email content")
            promise.succeed(true)
        } else {
            // Any other response is considered a failure
            logger.warning("DATA command rejected: \(response.code) \(response.message)")
            promise.succeed(false)
        }
        
        return true // Always complete after a single response
    }
} 